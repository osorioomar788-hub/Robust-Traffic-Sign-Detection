...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        # --- MODIFICACIÓN MICAI: Parámetros Focal Loss ---
        self.gamma = 2.0   # Factor de enfoque para señales difíciles
        self.alpha = 0.25  # Factor de balanceo de clases
        # -------------------------------------------------
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        # --- MODIFICACIÓN MICAI: Focal Loss Inyectada ---
        # 1. Calculamos la pérdida base
        bce_loss = self.bce(pred_scores, target_scores.to(dtype))
        
        # 2. Obtenemos las probabilidades de la red (Sigmoide)
        p = torch.sigmoid(pred_scores)
        
        # 3. Calculamos el factor modulador: |y - p|^gamma
        modulating_factor = torch.abs(target_scores.to(dtype) - p) ** self.gamma
        
        # 4. Aplicamos la fórmula final con el peso alfa
        focal_loss = self.alpha * modulating_factor * bce_loss
        
        # 5. Asignamos la nueva pérdida
        loss[1] = focal_loss.sum() / target_scores_sum 
        # ------------------------------------------------
